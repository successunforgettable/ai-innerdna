...
interface FoundationTowerProps {
  title: string;
  blocks: Array<{
    gradient: string;
    width: string;
...
export function FoundationTower({ title, blocks }: FoundationTowerProps) {
  const getWidthValue = (width: string) => {
    switch(width) {
      case 'w-48': return '16rem'; // Large foundation blocks
  ...
  const getHeightValue = (height: string) => {
    switch(height) {
      ...
      case 'h-8': return '3rem'; // Tall foundation blocks
