...
export async function generateHelper3Stage1Content(): Promise<Helper3StageContent> {
  try {
    const prompt = `
You are an expert in personality psychology specializing in Helper 3 personality patterns. Generate comprehensive Stage 1 "Ordinary World" content for a Helper 3 transformation report.
...
- Ready for direct HTML insertion
...
Return as valid JSON with this exact structure:
{
  ...
  "wheelOfLife": [
    ...
    console.log('AI-generated Helper 3 Stage 1 content:', {
      stageDescriptionLength: content.stageDescription?.length,
      challengeCardsCount: content.challengeCards?.length,
      wheelOfLifeCount: content.wheelOfLife?.length
    });
    ...
    throw new Error('Failed to generate Helper 3 content');
  }
}